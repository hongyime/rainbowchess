...
Hosted application/security checks passed. A second, legacy Labeler workflow failed because its default `.github/labeler.yml` was absent. It now explicitly uses the existing `.github/labels.yml`, matching the other successful label workflow, with read-contents/write-PR permissions. Final hosted checks follow this workflow fix.
